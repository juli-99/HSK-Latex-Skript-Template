%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  structure_presentation.tex -- Layout der Beamer-Praesentation
%
%  Arbeitsteilung:
%    presentation.tex            Metadaten, Fakultaetsfarbe, Folienreihenfolge
%    structure_presentation.tex  diese Datei: Folienlayout und \EIT...-Befehle
%    99_example.tex              Beispielfolien (gehoert nicht in die Abgabe)
%
%  Die Farbe ThemeColor muss VOR dem Einbinden dieser Datei definiert sein.
%
%  Die vier Hintergrund-PDFs des HKA-Corporate-Designs liegen im Ordner, den
%  \hkaTemplatePath angibt (Vorgabe: Templates/).
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%==============================================================================
%  PFAD ZU DEN HINTERGRUND-PDFS
%==============================================================================
% Frueher stand "Templates/" an vier Stellen fest verdrahtet. Ueber diesen
% Befehl laesst sich der Ordner an einer Stelle umhaengen.
\newcommand{\hkaTemplatePath}{Templates/}

%==============================================================================
%  SEITENFORMAT UND SCHRIFT
%==============================================================================
% inputenc entfaellt: UTF-8 ist seit 2018 LaTeX-Standard.
% hyperref bringt die Beamer-Klasse bereits mit.
...
% Intelligente Querverweise: \cref{eq:xyz} schreibt "Gl. (1)" -- das Wort
% davor entfaellt also. Muss nach hyperref stehen, das die Beamer-Klasse laedt.
...
%----------------------------------------------------------------------------------------
%	DIGITAL LOGIC & ELECTRONICS & PHYSICAL UNITS
%----------------------------------------------------------------------------------------
\usepackage[european, RPvoltages]{circuitikz}
\usepackage{xparse}
% siunitx wird weiter unten zusammen mit booktabs geladen -- eine Ladung genuegt.

% Deutsche Zahlkonventionen wie im Skript: Dezimalkomma statt Punkt (DIN 1333).
% Fuer eine englische Praesentation auf locale=US umstellen.

% Nur colortbl: das alte color-Paket wird nicht gebraucht, die Beamer-Klasse
% laedt bereits xcolor.
\usepackage{colortbl}
\usepackage{lipsum}
...
{
	\includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{\hkaTemplatePath HKA-Normal.pdf}
}
...
\newcommand{\defaultfoottext}{
	\begin{tikzpicture}[remember picture, overlay]
		\node[inner sep=0pt,outer sep=0, text width=\paperwidth, anchor=north west](a) at ($(current page.north west) + (\leftmarginpos, -17.15cm)$)
		{ \fontsize{9}{12}\selectfont\color{ThemeColor}
...
\newcommand{\customfoottext}{
	\begin{tikzpicture}[remember picture, overlay]
		\node[inner sep=0pt,outer sep=0, text width=\paperwidth, anchor=north west] at ($(current page.center) + (-16cm, -8.45cm)$)
		{ \fontsize{10}{12}\selectfont\color{ThemeColor}
			\insertframenumber
		};

		\node[inner sep=0pt,outer sep=0, text width=\textwidth, anchor=north] at ($(current page.center) + (0, -7.75cm)$)
		{ 	\begin{center}
			\fontsize{9}{12}\selectfont\color{ThemeColor}
			\foottext
			\end{center}
...
\newcommand \EITCoverVI[3]{
	\begin{frame}
		...
		\begin{tikzpicture}[remember picture, overlay]
			\node[at=(current page.center)]{\includegraphics[width=\paperwidth, height=\paperheight]{\hkaTemplatePath HKA-Opening.pdf}};
		...
		\begin{tikzpicture}[remember picture, overlay]
			\node[at=(current page.center)]{\includegraphics[width=\paperwidth, height=\paperheight]{\hkaTemplatePath HKA_Opening_KA_text.pdf}};
		\end{tikzpicture}
	\end{frame}
}

\newcommand \EITCoverSubtitleVI[4]{
	\begin{frame}

		\begin{tikzpicture}[remember picture, overlay]
			\node[at=(current page.center)]{\includegraphics[width=\paperwidth, height=\paperheight]{\hkaTemplatePath HKA-Opening.pdf}};

			\node[inner sep=0pt, text width=\paperwidth, anchor=south west](title) at ($(current page.center) + (-16cm, 3.825cm)$)
			{
				\fontsize{\covertextsize}{30}\selectfont\color{ThemeColor} #1
			};

			\node[below left = 0.958cm and 0cm of title, inner sep=0pt,outer sep=0, anchor=north west]
			{
				\fontsize{\coversubtitlesize}{30}\selectfont\color{ThemeColor} #2
			};
		\end{tikzpicture}
		...
		\begin{tikzpicture}[remember picture, overlay]
			\node[inner sep=0pt, anchor=north west] at ($(current page.west) + (0, 1.907cm)$)
			{
				\picdims[width=\paperwidth]{\paperwidth}{10.2cm}{#4}
			};

			\node[inner sep=0pt, text width=\paperwidth, anchor=south west] at ($(current page.center) + (-16cm, -9cm)$)
			{\fontsize{9.1}{5}\selectfont\color{ThemeColor} #3};
		\end{tikzpicture}
...
\newcommand \EITHeadline[2]{
	\begin{frame}
		...
		\begin{tikzpicture}[remember picture, overlay]
			\node[at=(current page.center)]{\includegraphics[width=\paperwidth, height=\paperheight]{\hkaTemplatePath HKA_Slide_KA_text.pdf}};
		\end{tikzpicture}
	\end{frame}
}
